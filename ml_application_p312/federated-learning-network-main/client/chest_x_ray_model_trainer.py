...
class ChestXRayModelTrainer:
    ...
    def train_model(self):
       ...
       #Next model ultra small:
        	#Total params: 422 (1.65 KB)
 		#Trainable params: 422 (1.65 KB)
 		#Non-trainable params: 0 (0.00 B)
        #model = Sequential([
        #    Conv2D(filters=1, kernel_size=(3, 3), activation='relu', padding='same', input_shape=(224, 224, 3)),
    	#    MaxPool2D(pool_size=(16, 16), strides=16),
        #    Flatten(),
        #    Dense(units=2, activation='softmax')
        #])
        #model.summary()
        ...
        cwd = os.getcwd()
        full_model_path = os.path.join(cwd+INITIAL_MODEL_PATH,MODEL)
        #print('full_model_path = ', full_model_path)
        #print('cwd = ', cwd)
        #print('cwd+INITIAL_MODEL_PATH = ', cwd+INITIAL_MODEL_PATH)
        #print('cwd+INITIAL_MODEL_PATH, MODEL = ', cwd+INITIAL_MODEL_PATH, MODEL) 
    ...
    def __load_datasets(self):
        ...
        #self.__build_training_dataset(global_dataset_train_path, training_dataset_train_path, ['NORMAL', 'PNEUMONIA'], 10)
        #self.__build_training_dataset(global_dataset_valid_path, training_dataset_valid_path, ['NORMAL', 'PNEUMONIA'], 10)
