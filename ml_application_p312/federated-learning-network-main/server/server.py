...
class Server:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(request_url, json=request_body) as response:
                if response.status != 200:
                    print('Error requesting training to client', training_client.client_url, 'response-status ', response.status)
    ...
    def update_server_model_params(self, training_type):
        if self.can_update_central_model_params():
            ...
            if training_type == TrainingType.MNIST:
            ...
            elif training_type == TrainingType.MNIST: #TrainingType.ORIG_CHEST_X_RAY_PNEUMONIA:
                print('TrainingType.CHEST_X_RAY_PNEUMONIA')
                received_weights = []
                print('received_weights = ', received_weights)
                for training_client in self.training_clients.values():
                    if training_client.status == ClientTrainingStatus.TRAINING_FINISHED:
                        training_client.status = ClientTrainingStatus.IDLE
                        received_weights.append(training_client.model_params)
                        #print('received_weights_appened = ', received_weights )
                        #print('size received_weights 0 ', len(received_weights))
                ...
                print('TrainingType.NEW_CHEST_X_RAY_PNEUMONIA')
                received_weights = []
                #num_layers = len(received_weights_appended[0])
		#num_clients = len(received_weights_appended)
                #print('received_weights = ', received_weights)
                for training_client in self.training_clients.values():
                    if training_client.status == ClientTrainingStatus.TRAINING_FINISHED:
                        training_client.status = ClientTrainingStatus.IDLE
                        received_weights.append(training_client.model_params)
                        #print('received_weights_appened = ', received_weights )
                #print('size received_weights ', len(received_weights))
                #new_weights = np.stack(received_weights).mean(0)
                new_weights = self.average_multiple_clients(received_weights)
                #new_weights = received_weights
                self.chest_x_ray_model_params = new_weights
                print('Model weights for', TrainingType.CHEST_X_RAY_PNEUMONIA, 'updated in central model')
            self.status = ServerStatus.IDLE
        sys.stdout.flush()

    def average_multiple_clients(self, received_weights):
        num_clients = len(received_weights)
        num_layers = len(received_weights[0])
        print('num_clients ', len(received_weights))
        print('num_layers ', len(received_weights[0]))
        ...
        # Get this layer's weights from all clients
            ...
            layer_weights_from_all_clients = []
            for client_idx in range(num_clients):
                 #print('client_idx ', client_idx)
                 layer_weights_from_all_clients.append(received_weights[client_idx][layer_idx])
        
        # Average this layer
            averaged_layer = np.stack(layer_weights_from_all_clients).mean(axis=0)
            averaged_weights.append(averaged_layer)
        
        #print('averaged_weights ', averaged_weights)
